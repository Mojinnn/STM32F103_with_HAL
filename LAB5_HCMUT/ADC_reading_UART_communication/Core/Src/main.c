...
uint32_t lastADC_value = 0;
uint8_t response[MAX_BUFFER_SIZE];

int failure = 0;
int parser_state = IDLE;
int cmd_state = NONE;
int cmd_flag = 0;
int cursor_cmd = 0;
int ok_flag = 0;
...
void uart_communication_fsm () {
	...
	case IDLE:
		if (cmd_flag == 1) {
			cmd_flag = 0;
			uart_state = CMD_CHECK;
		}
		break;
	case CMD_CHECK:
		if (cmd_state == RST) uart_state = SEND;
		else if (cmd_state == OK) {
			uart_state = IDLE;
			HAL_GPIO_WritePin(Blink_led_GPIO_Port, Blink_led_Pin, SET);
		}
		break;
	case ERROR_FORMAT:
		snprintf((char*) response, sizeof(response), "Wrong Format \r\n");
		HAL_UART_Transmit(&huart2, response, sizeof(response), 1000);
		uart_state = IDLE;
		break;
	case SEND:
		HAL_GPIO_WritePin(Blink_led_GPIO_Port, Blink_led_Pin, RESET);
		ADC_value = HAL_ADC_GetValue(&hadc1);
		lastADC_value = ADC_value;
		sniprintf((char*)response, sizeof(response), "ADC=%lu#\r\n", (uint32_t)ADC_value);
		HAL_UART_Transmit(&huart2, response, sizeof(response), 1000);
		uart_state = WAIT_ACK;
		failure = 0;
...
void command_parser_fsm() {
	...
	case RECEIVING:
		if (buffer_flag == 1) {
			if (character == '#') {			// end command
				cmd[cursor_cmd] = '\0';		// add NULL to end string
				parser_state = DONE;
			}
			else {
				cmd[cursor_cmd++] = character;
			}
		}
		break;
	case DONE:
